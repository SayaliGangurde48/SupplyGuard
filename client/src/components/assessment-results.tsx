...
import { RefreshCw, FileText, Clock, DollarSign, Calendar } from "lucide-react";
...
export default function AssessmentResults({ assessment, onProcessingComplete }: AssessmentResultsProps) {
  ...
  const exportToPDF = () => {
    const vulnerabilitiesText = assessment.vulnerabilities && Array.isArray(assessment.vulnerabilities) 
      ? (assessment.vulnerabilities as Vulnerability[]).map(v => 
          `- ${v.title} (${v.severity}): ${v.description}`
        ).join('\n')
      : 'No vulnerabilities identified';
    
    const recommendationsText = assessment.recommendations && Array.isArray(assessment.recommendations)
      ? (assessment.recommendations as Recommendation[]).map(r => 
...
Generated: ${formatDate(assessment.updatedAt)}

RISK SCORES:
- Overall Risk: ${assessment.overallRiskScore?.toFixed(1) || 'N/A'}
- Supplier Risk: ${assessment.supplierRiskScore?.toFixed(1) || 'N/A'}
- Logistics Risk: ${assessment.logisticsRiskScore?.toFixed(1) || 'N/A'}
- Geopolitical Risk: ${assessment.geopoliticalRiskScore?.toFixed(1) || 'N/A'}
...
Report generated by Supply Chain Guardian AI`;
    
    const blob = new Blob([content], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `vulnerability-assessment-${assessment.companyName}-${new Date().toISOString().split('T')[0]}.txt`;
          ...
          <div className="mb-8">
            ...
            <div className="space-y-4">
              {(assessment.vulnerabilities as Vulnerability[]).map((vulnerability) => (
                <Card key={vulnerability.id} className={`border-${getSeverityColor(vulnerability.severity) === 'destructive' ? 'red' : getSeverityColor(vulnerability.severity) === 'secondary' ? 'orange' : 'gray'}-200 bg-${getSeverityColor(vulnerability.severity) === 'destructive' ? 'red' : getSeverityColor(vulnerability.severity) === 'secondary' ? 'orange' : 'gray'}-50`}>
